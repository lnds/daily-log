...
"#,
        ...
        pub fn create_doing_file_with_entries(&self, entries: Vec<TestEntry>) -> color_eyre::Result<()> {
            let mut sections: HashMap<String, Vec<String>> = HashMap::new();
            
            for entry in entries {
                let section = entry.section.unwrap_or("Currently".to_string());
                let mut line = format!(" - {} | {}", format_date(entry.timestamp), entry.description);
                ...
                sections.entry(section.clone()).or_insert_with(Vec::new).push(line);
                
                if let Some(note) = entry.note {
                    for note_line in note.lines() {
                        sections.get_mut(&section).unwrap().push(format!("  {}", note_line));
                    }
                }
            }

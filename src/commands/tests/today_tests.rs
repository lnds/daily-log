...
mod tests {
    ...
    fn test_today_shows_only_todays_entries() -> color_eyre::Result<()> {
        ...
        let today_morning = now.date_naive().and_hms_opt(9, 0, 0).unwrap()
            .and_local_timezone(Local).unwrap();
        let today_afternoon = now.date_naive().and_hms_opt(14, 0, 0).unwrap()
            .and_local_timezone(Local).unwrap();
        
        let entries = vec![
            TestEntry::new("Yesterday's task")
                .with_timestamp(yesterday),
            TestEntry::new("Morning task")
                .with_timestamp(today_morning),
            TestEntry::new("Afternoon task")
                .with_timestamp(today_afternoon),
            TestEntry::new("Just now task")
                .with_timestamp(now),
        ];
    ...
    fn test_today_with_done_entries() -> color_eyre::Result<()> {
        ...
        let morning = now.date_naive().and_hms_opt(9, 0, 0).unwrap()
            .and_local_timezone(Local).unwrap();
        let noon = now.date_naive().and_hms_opt(12, 0, 0).unwrap()
            .and_local_timezone(Local).unwrap();
    ...
    fn test_today_handles_midnight_boundary() -> color_eyre::Result<()> {
        ...
        let now = Local::now();
        let today_start = now.date_naive().and_hms_opt(0, 0, 1).unwrap()
            .and_local_timezone(Local).unwrap();
        ...
        let tomorrow_start = (now.date_naive() + Duration::days(1))
            .and_hms_opt(0, 0, 1).unwrap()
            .and_local_timezone(Local).unwrap();
        
        let entries = vec![
            TestEntry::new("Yesterday before midnight")
                .with_timestamp(yesterday_end),
            TestEntry::new("Today just after midnight")
                .with_timestamp(today_start),
            TestEntry::new("Current task")
                .with_timestamp(now),
            TestEntry::new("Tomorrow's task")
                .with_timestamp(tomorrow_start),
        ];
        ctx.create_doing_file_with_entries(entries)?;

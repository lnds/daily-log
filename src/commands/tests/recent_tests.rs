...
mod tests {
    ...
    fn test_recent_with_done_entries() -> color_eyre::Result<()> {
        ...
        let entries = vec![
            ...
            TestEntry::new("Ongoing task").with_timestamp(now - Duration::hours(1)),
    ...
    fn test_recent_sorts_by_timestamp() -> color_eyre::Result<()> {
        ...
        let now = Local::now();
        let entries = vec![
            TestEntry::new("Middle task").with_timestamp(now - Duration::hours(2)),
            TestEntry::new("Oldest task").with_timestamp(now - Duration::hours(3)),
            TestEntry::new("Newest task").with_timestamp(now - Duration::hours(1)),
        ];
        ctx.create_doing_file_with_entries(entries)?;

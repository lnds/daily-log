...
mod tests {
    ...
    fn test_now_simple_entry() -> color_eyre::Result<()> {
        ...
        let re =
            Regex::new(r"- \d{4}-\d{2}-\d{2} \d{2}:\d{2} \| Working on tests @?.*<[a-f0-9-]+>")?;
    ...
    fn test_now_with_back_flag() -> color_eyre::Result<()> {
        ...
        assert!(
            time_diff < 2,
            "Entry should be backdated by approximately 2 hours"
        );
    ...
    fn test_now_with_finish_last() -> color_eyre::Result<()> {
        ...
        let prev_entry = entries
            .iter()
            .find(|e| e.description == "Previous task")
            .unwrap();
        assert!(
            prev_entry.is_done(),
            "Previous entry should be marked as done"
        );

        // New task should exist and not be done
        let new_entry = entries
            .iter()
            .find(|e| e.description == "New task")
            .unwrap();
        assert!(
            !new_entry.is_done(),
            "New entry should not be marked as done"
        );
    ...
    fn test_now_with_section() -> color_eyre::Result<()> {
        ...
        println!(
            "Sections: {:?}",
            doing_file.sections.keys().collect::<Vec<_>>()
        );

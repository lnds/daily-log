...
mod tests {
    ...
    fn test_done_marks_last_entry() -> color_eyre::Result<()> {
        ...
        let last_entry = entries.iter().find(|e| e.description == "Last task").unwrap();
        assert!(last_entry.is_done());
        
        // Others should not be done
        let first_entry = entries.iter().find(|e| e.description == "First task").unwrap();
    ...
    fn test_done_creates_new_entry() -> color_eyre::Result<()> {
        ...
        let entry = entries.iter().find(|e| e.description == "Quick task").unwrap();
    ...
    fn test_done_with_at_time() -> color_eyre::Result<()> {
        ...
        let now = Local::now();
        let entries = vec![
            TestEntry::new("Task to complete").with_timestamp(now - Duration::hours(3)),
        ];
        ctx.create_doing_file_with_entries(entries)?;
        ...
        let entry = doing_file.sections.get("Currently").unwrap()
            .iter().find(|e| e.description == "Task to complete").unwrap();
    ...
    fn test_done_with_took_duration() -> color_eyre::Result<()> {
        ...
        let now = Local::now();
        let entries = vec![
            TestEntry::new("Task that took time")
        ...
        let doing_file = parse_taskpaper(&ctx.doing_file_path)?;
        let entry = doing_file.sections.get("Currently").unwrap()
            .iter().find(|e| e.description == "Task that took time").unwrap();
    ...
    fn test_done_remove_flag() -> color_eyre::Result<()> {
        ...
        let doing_file = parse_taskpaper(&ctx.doing_file_path)?;
        let entry = doing_file.sections.get("Currently").unwrap()
            .iter().find(|e| e.description == "Done task").unwrap();
    ...
    fn test_done_with_archive() -> color_eyre::Result<()> {
        ...
        let now = Local::now();
        let entries = vec![
            TestEntry::new("Task to archive").with_timestamp(now),
        ];
        ctx.create_doing_file_with_entries(entries)?;
        ...
        assert!(!currently_entries.iter().any(|e| e.description == "Task to archive"));
        
        // Should be in Archive
        let archive_entries = doing_file.sections.get("Archive").unwrap();
        let entry = archive_entries.iter().find(|e| e.description == "Task to archive").unwrap();
        assert!(entry.is_done());

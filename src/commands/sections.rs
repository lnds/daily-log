...
fn list_sections(doing_file: &DoingFile, column: bool) {
    ...
    let sections: BTreeMap<&String, usize> = doing_file.sections
        .iter()
    ...
    if column {
        ...
        let max_name_len = sections.keys()
            .map(|name| name.len())
            .max()
            .unwrap_or(0);
...
fn add_section(doing_file: &mut DoingFile, section_name: &str) -> Result<()> {
    if doing_file.sections.contains_key(section_name) {
        return Err(color_eyre::eyre::eyre!("Section '{}' already exists", section_name));
    }

    doing_file.sections.insert(section_name.to_string(), Vec::new());
    Ok(())
}

fn remove_section(doing_file: &mut DoingFile, section_name: &str, archive: bool) -> Result<()> {
    if !doing_file.sections.contains_key(section_name) {
        return Err(color_eyre::eyre::eyre!("Section '{}' does not exist", section_name));
    }

    // Don't allow removing the default section
    if section_name == "Currently" {
        return Err(color_eyre::eyre::eyre!("Cannot remove the 'Currently' section"));
    }
